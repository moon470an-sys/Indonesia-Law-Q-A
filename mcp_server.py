...
venv는 OneDrive 외부에 둔다 (자세한 이유는 auto_start/watchdog.ps1 주석 참고).
기본 경로: D:\\venvs\\rag_indonesia_law (RAG_VENV_DIR 환경변수로 override).

준비:
  D:\\venvs\\rag_indonesia_law\\Scripts\\pip install mcp  (1.27+ 권장)

수동 테스트 (stdio echo):
  python mcp_server.py
  → Claude Desktop/Code가 호출할 stdio 프로토콜 대기. Ctrl+C로 종료.

Claude Desktop 등록 (Windows): scripts/install_desktop_mcp.py 한 번 실행하면
%LOCALAPPDATA%\\Packages\\Claude_pzs8sxrjxfjjc\\LocalCache\\Roaming\\Claude\\
claude_desktop_config.json에 자동으로 등록됨. 등록되는 모양:
  {
    "mcpServers": {
      "indonesia-law-rag": {
        "command": "D:\\\\venvs\\\\rag_indonesia_law\\\\Scripts\\\\python.exe",
        "args": ["C:\\\\Users\\\\yoonseok.moon\\\\OneDrive - ...\\\\mcp_server.py"]
      }
    }
  }
Claude Code는 같은 모양을 .mcp.json (repo 루트)에서 읽음.
"""
